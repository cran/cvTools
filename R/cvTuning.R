...
cvTuning.function <- function(object, formula, data = NULL, x = NULL, y, 
    ...
    out <- cvTuning(call, data=data, x=x, y=y, tuning=tuning, cost=cost, 
        K=K, R=R, folds=folds, names=names, predictArgs=predictArgs, 
...
cvTuning.call <- function(object, data = NULL, x = NULL, y, tuning = list(), 
        cost = rmspe, K = 5, R = 1, folds = NULL, names = NULL, 
    ...
    if(nTuning == 0 || pTuning == 0) {
        ...
        out <- cvFit(object, data, x, y, cost=cost, K=K, R=R, folds=folds, 
